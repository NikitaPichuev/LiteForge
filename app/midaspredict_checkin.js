...
const LOG_FILE = path.join(
  LOGS_DIR,
  `midaspredict_checkin_${new Date().toISOString().replace(/[-:]/g, "").slice(0, 15).replace("T", "_")}.log`
);
...
function usage() {
  console.log(`
MidasPredict daily check-in

Commands:
  node app\\midaspredict_checkin.js --all --send
  node app\\midaspredict_checkin.js --wallet-index 1 --send

Notes:
  Check-in API requires auth. The script signs the Midas auth message with keys.txt.
  Without --send it only prints planned wallets.
`);
}
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i];
    if (arg === "--wallet-index") parsed.walletIndex = Number.parseInt(args[++i], 10);
    ...
    const req = https.request(url, { method, headers }, (res) => {
      ...
      res.on("end", () => {
        ...
        if (res.statusCode < 200 || res.statusCode >= 300) {
          ...
          reject(new Error(`${method} ${url.pathname} failed: HTTP ${res.statusCode} ${msg}`));
          return;
        }
        resolve(parsed);
      });
...
async function getLoginStreak(authToken) {
  return requestJson("GET", "/login/streak", null, authToken);
}

async function checkIn(authToken) {
  return requestJson("POST", "/login/checkin", {}, authToken);
}
...
async function main() {
  ...
  if (!parsed.send) {
    for (const item of selected) {
      ...
      log("INFO", `[wallet ${item.index}] dry-run check-in target: ${wallet.address}`);
    }
    log("INFO", "Dry-run only. Add --send to run check-in.");
    return;
  }
  ...
  for (let i = 0; i < selected.length; i += 1) {
    ...
    try {
      const before = await getLoginStreak(token).catch((error) => ({ success: false, error: error.message }));
      log("INFO", `[wallet ${item.index}] streak before: ${compactJson(before)}`);
      const result = await checkIn(token);
      ok += 1;
      log("INFO", `[wallet ${item.index}] check-in response: ${compactJson(result)}`);
      const after = await getLoginStreak(token).catch((error) => ({ success: false, error: error.message }));
      log("INFO", `[wallet ${item.index}] streak after: ${compactJson(after)}`);
    } catch (error) {
      failed += 1;
      log("WARNING", `[wallet ${item.index}] check-in failed: ${error.message}`);
    }
  ...
  log("INFO", `Summary: ok=${ok}, skipped=${skipped}, failed=${failed}`);
}
